...
* 如果你对线段树完全不了解，建树时有一些疑惑，看本文有点不理解，可以试试先看一下 [线段树啊建树方法与原理](http://blog.zinchon.cn/2019/09/SegmentTree/)
* 如果你已经对于线段树的基础操作非常了解，请直接进入下一篇文章 [线段树 + 离散化](https://hybrogen.github.io/2020/03/SegmentTree3/)
...
那么对于这些操作，我们先来看一下第一个操作应该如何进行。由于我们要 **维护** 线段树，而上一节我们讲过线段树除了叶子节点，每个节点的值都是它所分出来的两个子节点的和，所以我们在改变叶子节点的值的时候要同时更新与它有关的节点（父节点）的信息。<br>
...
3. 如果这个区间的右子区间和目标区间有交集，那么搜索右子区间
...
[A Simple Problem with Integers](http://poj.org/problem?id=3468)<br>
